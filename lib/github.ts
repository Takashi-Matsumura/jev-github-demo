...
// ─── AI co-author 検出 ──────────────────────────────────────────────
// Co-authored-by: トレーラは GitHub 側が Commit.authors として構造化済みで返す
// （自前パース・追加リクエスト不要）。email / login のいずれかが一致すれば検出する。
type CommitAuthorLike = { login: string | null; email: string | null };
const AGENT_PATTERNS: { name: string; test: (a: CommitAuthorLike) => boolean }[] = [
  { name: "Claude", test: (a) => a.login === "claude" || /noreply@anthropic\.com$/i.test(a.email ?? "") },
  { name: "GitHub Copilot", test: (a) => /^copilot-swe-agent(\[bot\])?$/i.test(a.login ?? "") },
  { name: "Devin", test: (a) => /^devin-ai-integration(\[bot\])?$/i.test(a.login ?? "") },
  { name: "Cursor", test: (a) => /^cursoragent$/i.test(a.login ?? "") },
];

function detectAgentsInCommit(authors: CommitAuthorLike[]): string[] {
  const found = new Set<string>();
  for (const a of authors) {
    for (const p of AGENT_PATTERNS) {
      if (p.test(a)) found.add(p.name);
    }
  }
  return [...found];
}

// PR body 側の生成マーカー。コミット単位の co-author が取れない REST 経路や、
// トークン無しの環境でも body だけは既に取得済みなので副次ソースとして使える。
const BODY_AGENT_PATTERNS: { name: string; re: RegExp }[] = [
  { name: "Claude", re: /generated with \[claude code\]|co-authored-by:\s*claude/i },
  { name: "GitHub Copilot", re: /generated by (github )?copilot/i },
  { name: "Devin", re: /generated by devin/i },
];

function detectAgentsInBody(body: string | null): string[] {
  if (!body) return [];
  const found = new Set<string>();
  for (const p of BODY_AGENT_PATTERNS) {
    if (p.re.test(body)) found.add(p.name);
  }
  return [...found];
}
  ...
  repository(owner: $owner, name: $name) {
    ...
    pullRequests(first: $first, after: $after, states: [MERGED, CLOSED],
      ...
      nodes {
        ...
        commits(first: 100) {
          totalCount
          nodes { commit { messageHeadline authors(first: 5) { nodes { name email user { login } } } } }
        }
        reviews(first: 20) {
          totalCount
          nodes {
  ...
  commits: {
    ...
    nodes: {
      commit: {
        ...
        authors: { nodes: { name: string | null; email: string | null; user: { login: string } | null }[] };
      };
    }[];
  };
  reviews: {
...
function mapGqlNode(node: GqlPrNode): FetchedPr {
  ...
  for (const c of node.commits.nodes) {
    const authors = c.commit.authors.nodes.map((a) => ({ login: a.user?.login ?? null, email: a.email }));
    const found = detectAgentsInCommit(authors);
    if (found.length > 0) {
      aiCommitCount += 1;
      for (const a of found) agents.add(a);
    }
  }
  let aiSource: "trailer" | "body" | "none" = "none";
  if (aiCommitCount > 0) {
    aiSource = "trailer";
  } else {
    const bodyAgents = detectAgentsInBody(node.body);
    if (bodyAgents.length > 0) {
      aiSource = "body";
      for (const a of bodyAgents) agents.add(a);
    }
  }
  ...
  return {
    ...
    commitsTruncated: node.commits.totalCount > node.commits.nodes.length,
    aiCommitCount,
    aiAgents: [...agents],
    aiSource,
...
async function* fetchViaRest(opts: FetchOptions): AsyncGenerator<FetchedPr> {
  ...
  for (const summary of summaries.slice(0, cappedLimit)) {
    ...
    const bodyAgents = detectAgentsInBody(detail.body);
    ...
    const pr: FetchedPr = {
      ...
      // REST 経路はコミット単位の co-author を取得しない（GITHUB_TOKEN 未設定時のみ通る経路）。
      // body の生成マーカーだけを副次ソースとして見る。
      aiCommitCount: 0,
      aiAgents: bodyAgents,
      aiSource: bodyAgents.length > 0 ? "body" : "unmeasured",
